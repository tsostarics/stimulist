...
add_stims <- function(design,
  ...
  num_trials <- attr(design$trials, 'total')

  names_stims <- names(stims)
  names_trials <- names(design[['trials']])

  if (all(length(design[['trials']]) != 0)) {
    if (any(names_stims != names_trials)){
      if (all(sort(names_stims) == sort(names_trials)))
  ...
  attr(design$trials, 'total') <- num_stims
  design
}
